    ...
    public SortMerge(AttrType in1[],
        ...
        if (!in1_sorted) {
            try {
                ...
                bigt.orderType = 1;
                p_i1 = new SortMap(bigt.BIGT_ATTR_TYPES, (short) 0,bigt.BIGT_STR_SIZES, am1, join_col_in1, order,sortFld1Len, amt_of_mem / 2);
            } catch (Exception e) {
        ...
        if (!in2_sorted) {
            try {
                bigt.mapInsertOrder = false;
                bigt.orderType = 1;
                p_i2 = new SortMap(bigt.BIGT_ATTR_TYPES, (short) 0, bigt.BIGT_STR_SIZES, am2, join_col_in2, order,sortFld2Len, amt_of_mem/2);
            } catch (Exception e) {
                throw new SortException(e, "Sort failed");
            }
        }
        ...
        try {
            TempMap1.setHdr((short) 0, _in1, s1_sizes); //ask
            map1.setHdr((short) 0,_in1, s1_sizes);//ask
            TempMap2.setHdr((short) 0,_in2, s2_sizes);//ask
            map2.setHdr((short) 0,_in2, s2_sizes);//ask
        } catch (Exception e) {
